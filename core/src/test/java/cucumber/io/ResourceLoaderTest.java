...
public class ResourceLoaderTest {
    ...
    public void loads_resources_from_filesystem_dir() {
        Iterator<Resource> files = new FileResourceLoader().resources(dir.getAbsolutePath(), ".properties");
    ...
    public void loads_resource_from_filesystem_file() {
        ...
        Iterator<Resource> files = new FileResourceLoader().resources(file.getPath(), ".doesntmatter");
        assertEquals(1, toList(files).size());
    }

    @Test
    public void loads_resources_from_jar_on_classpath() throws IOException {
        Iterator<Resource> files = new ClasspathResourceLoader(Thread.currentThread().getContextClassLoader()).resources("cucumber", ".properties");
